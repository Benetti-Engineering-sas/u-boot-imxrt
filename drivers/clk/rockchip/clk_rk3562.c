...
#define RK3562_CPUCLK_RATE(_rate, _aclk_div, _pclk_div)		\
{								\
	.rate	= _rate##U,					\
	.aclk_div = _aclk_div,					\
	.pclk_div = _pclk_div,					\
...
static struct rockchip_cpu_rate_table rk3562_cpu_rates[] = {
	RK3562_CPUCLK_RATE(1416000000, 1, 5),
	RK3562_CPUCLK_RATE(1296000000, 1, 5),
	RK3562_CPUCLK_RATE(1200000000, 1, 3),
	RK3562_CPUCLK_RATE(1104000000, 1, 3),
	RK3562_CPUCLK_RATE(1008000000, 1, 3),
	RK3562_CPUCLK_RATE(912000000, 1, 3),
	RK3562_CPUCLK_RATE(816000000, 1, 3),
	RK3562_CPUCLK_RATE(600000000, 1, 1),
	RK3562_CPUCLK_RATE(408000000, 1, 1),
	{ /* sentinel */ },
};
...
static ulong rk3562_pmu1cru_get_rate(struct clk *clk)
{
	...
	case CLK_I2C0:
		...
		break;
	case CLK_PWM0:
		rate = rk3562_pwm_get_pmuclk(priv, clk->id);
		break;
	case PCLK_PMU:
		rate = rk3562_pmu_get_pmuclk(priv);
		break;
	default:
		return -ENOENT;
	}
...
static ulong rk3562_pmu1cru_set_rate(struct clk *clk, ulong rate)
{
	...
	case CLK_I2C0:
		...
		break;
	case CLK_PWM0:
		ret = rk3562_pwm_set_pmuclk(priv, clk->id, rate);
		break;
	case PCLK_PMU:
...
static int rk3562_armclk_set_clk(struct rk3562_clk_priv *priv, ulong hz)
{
	...
	rate = rockchip_get_cpu_settings(rk3562_cpu_rates, hz);
	if (!rate) {
	 ...
	 */
	old_rate = rockchip_pll_get_rate(&rk3562_pll_clks[APLL],
					 priv->cru, APLL);
	if (old_rate > hz) {
				...
				ulong clk_id, ulong rate)
{
	...
	return rk3562_bus_get_clk(priv, clk_id);
}
				    ...
				    ulong clk_id, ulong rate)
{
	...
	return rk3562_perimid_get_clk(priv, clk_id);
}
				...
				ulong clk_id, ulong rate)
{
	...
	return rk3562_top_get_clk(priv, clk_id);
}
				...
				ulong rate)
{
	...
	return rk3562_i2c_get_clk(priv, clk_id);
}
				...
				ulong clk_id, ulong rate)
{
	...
	return rk3562_spi_get_clk(priv, clk_id);
}
				...
				ulong clk_id, ulong rate)
{
	...
	return rk3562_pwm_get_clk(priv, clk_id);
}
...
static ulong rk3562_adc_get_clk(struct rk3562_clk_priv *priv, ulong clk_id)
{
	...
	case CLK_TSADC:
		...
		prate = rk3562_adc_get_clk(priv, CLK_TSADC_TSEN);
		return DIV_TO_RATE(prate, div);
	default:
		return -ENOENT;
	}
				...
				ulong clk_id, ulong rate)
{
	...
	return rk3562_adc_get_clk(priv, clk_id);
}
				  ...
				  ulong clk_id, ulong rate)
{
	...
	return rk3562_sdmmc_get_clk(priv, clk_id);
}
...
static ulong rk3562_sfc_set_clk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_sfc_get_clk(priv);
}
...
static ulong rk3562_nand_set_clk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_nand_get_clk(priv);
}
...
static ulong rk3562_emmc_set_clk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_emmc_get_clk(priv);
}
...
static ulong rk3562_emmc_set_bclk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_emmc_get_bclk(priv);
}
...
static ulong rk3562_aclk_vop_set_clk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_aclk_vop_get_clk(priv);
}
...
static ulong rk3562_dclk_vop_get_clk(struct rk3562_clk_priv *priv, ulong clk_id)
{
	...
	if (sel == DCLK_VOP_SEL_HPLL)
		parent = rk3562_pmu_pll_get_rate(priv, HPLL);
	else if (sel == DCLK_VOP_SEL_VPLL)
		parent = rockchip_pll_get_rate(&rk3562_pll_clks[VPLL],
					       priv->cru, VPLL);
				     ...
				     ulong clk_id, ulong rate)
{
	...
	if (sel == DCLK_VOP_SEL_HPLL) {
		...
		rk3562_pmu_pll_set_rate(priv, HPLL, div * rate);
	} else if (sel == DCLK_VOP_SEL_VPLL) {
		...
		rk_clrsetreg(&cru->clksel_con[conid],
			     ...
			     ((div - 1) << DCLK0_VOP_DIV_SHIFT));
		rockchip_pll_set_rate(&rk3562_pll_clks[VPLL],
	...
	return rk3562_dclk_vop_get_clk(priv, clk_id);
}
				     ...
				     ulong mac_id)
{
	...
	case CLK_MAC0_2TOP_SEL_PPLL:
		return rk3562_pmu_pll_get_rate(priv, HPLL);
	default:
		return -ENOENT;
	}
}
				     ...
				     ulong mac_id, ulong rate)
{
	...
	return rk3562_gmac_src_get_clk(priv, mac_id);
}
				     ...
				     ulong mac_id, ulong rate)
{
	...
	return rk3562_gmac_out_get_clk(priv, mac_id);
}
					 ...
					 ulong mac_id, ulong rate)
{
	...
	return rk3562_gmac_ptp_ref_get_clk(priv, mac_id);
}
...
static ulong rk3562_ebc_set_clk(struct rk3562_clk_priv *priv, ulong rate)
{
	...
	return rk3562_ebc_get_clk(priv);
}
				   ...
				   ulong clk_id, ulong rate)
{
	...
	return rk3562_rkvdec_get_clk(priv, clk_id);
}
#endif
...
static ulong rk3562_clk_get_rate(struct clk *clk)
{
	...
	case PCLK_BUS:
	case PCLK_WDT_NS:
		rate = rk3562_bus_get_clk(priv, clk->id);
		break;
	case ACLK_PERIMID:
	case HCLK_PERIMID:
		rate = rk3562_perimid_get_clk(priv, clk->id);
		break;
	case ACLK_TOP_HIGH:
	case ACLK_TOP_LOW:
	case HCLK_TOP:
	case PCLK_TOP:
		rate = rk3562_top_get_clk(priv, clk->id);
		break;
	case CLK_I2C1:
	case CLK_I2C2:
	case CLK_I2C3:
	case CLK_I2C4:
	case CLK_I2C5:
		rate = rk3562_i2c_get_clk(priv, clk->id);
		break;
	case CLK_SPI0:
	case CLK_SPI1:
	case CLK_SPI2:
	case CLK_SPI3:
		rate = rk3562_spi_get_clk(priv, clk->id);
		break;
	case CLK_PWM1:
	case CLK_PWM2:
	case CLK_PWM3:
		rate = rk3562_pwm_get_clk(priv, clk->id);
		break;
	case CLK_SARADC:
	case CLK_TSADC_TSEN:
	case CLK_TSADC:
		rate = rk3562_adc_get_clk(priv, clk->id);
		break;
	case HCLK_SDMMC0:
	case CLK_SDMMC0:
	case CLK_SDMMC1:
	case CLK_SDMMC2:
		rate = rk3562_sdmmc_get_clk(priv, clk->id);
		break;
	case SCLK_SFC:
		rate = rk3562_sfc_get_clk(priv);
		break;
	case NCLK_NANDC:
		rate = rk3562_nand_get_clk(priv);
		break;
	case CCLK_EMMC:
		rate = rk3562_emmc_get_clk(priv);
		break;
	case BCLK_EMMC:
		rate = rk3562_emmc_get_bclk(priv);
		break;
	case TCLK_EMMC:
		rate = OSC_HZ;
		break;
#ifndef CONFIG_SPL_BUILD
	case ACLK_VOP:
		rate = rk3562_aclk_vop_get_clk(priv);
		break;
	case DCLK_VOP0:
	case DCLK_VOP1:
	case DCLK_VOP2:
		rate = rk3562_dclk_vop_get_clk(priv, clk->id);
		break;
	case SCLK_GMAC0:
	case CLK_MAC0_2TOP:
	case CLK_MAC0_REFOUT:
		rate = rk3562_gmac_src_get_clk(priv, 0);
		break;
	case CLK_MAC0_OUT:
		rate = rk3562_gmac_out_get_clk(priv, 0);
		break;
	case CLK_GMAC0_PTP_REF:
		rate = rk3562_gmac_ptp_ref_get_clk(priv, 0);
		break;
	case SCLK_GMAC1:
	case CLK_MAC1_2TOP:
	case CLK_MAC1_REFOUT:
		rate = rk3562_gmac_src_get_clk(priv, 1);
		break;
	case CLK_MAC1_OUT:
		rate = rk3562_gmac_out_get_clk(priv, 1);
		break;
	case CLK_GMAC1_PTP_REF:
		rate = rk3562_gmac_ptp_ref_get_clk(priv, 1);
		break;
	case DCLK_EBC:
		rate = rk3562_ebc_get_clk(priv);
		break;
	case ACLK_RKVDEC_PRE:
	case ACLK_RKVDEC:
	case CLK_RKVDEC_CORE:
		rate = rk3562_rkvdec_get_clk(priv, clk->id);
		break;
	case TCLK_WDT_NS:
		rate = OSC_HZ;
		break;
...
static ulong rk3562_clk_set_rate(struct clk *clk, ulong rate)
{
	...
	case PCLK_BUS:
	case PCLK_WDT_NS:
		ret = rk3562_bus_set_clk(priv, clk->id, rate);
		break;
	case ACLK_PERIMID:
	case HCLK_PERIMID:
		ret = rk3562_perimid_set_clk(priv, clk->id, rate);
		break;
	case ACLK_TOP_HIGH:
	case ACLK_TOP_LOW:
	case HCLK_TOP:
	case PCLK_TOP:
		ret = rk3562_top_set_clk(priv, clk->id, rate);
		break;
	case CLK_I2C1:
	case CLK_I2C2:
	case CLK_I2C3:
	case CLK_I2C4:
	case CLK_I2C5:
		ret = rk3562_i2c_set_clk(priv, clk->id, rate);
		break;
	case CLK_SPI0:
	case CLK_SPI1:
	case CLK_SPI2:
	case CLK_SPI3:
		ret = rk3562_spi_set_clk(priv, clk->id, rate);
		break;
	case CLK_PWM1:
	case CLK_PWM2:
	case CLK_PWM3:
		ret = rk3562_pwm_set_clk(priv, clk->id, rate);
		break;
	case CLK_SARADC:
	case CLK_TSADC_TSEN:
	case CLK_TSADC:
		ret = rk3562_adc_set_clk(priv, clk->id, rate);
		break;
	case HCLK_SDMMC0:
	case CLK_SDMMC0:
	case CLK_SDMMC1:
	case CLK_SDMMC2:
		ret = rk3562_sdmmc_set_clk(priv, clk->id, rate);
		break;
	case SCLK_SFC:
		ret = rk3562_sfc_set_clk(priv, rate);
		break;
	case NCLK_NANDC:
		ret = rk3562_nand_set_clk(priv, rate);
		break;
	case CCLK_EMMC:
		ret = rk3562_emmc_set_clk(priv, rate);
		break;
	case BCLK_EMMC:
		ret = rk3562_emmc_set_bclk(priv, rate);
		break;
	case TCLK_EMMC:
		ret = OSC_HZ;
		break;
#ifndef CONFIG_SPL_BUILD
	case ACLK_VOP:
		ret = rk3562_aclk_vop_set_clk(priv, rate);
		break;
	case DCLK_VOP0:
	case DCLK_VOP1:
	case DCLK_VOP2:
		ret = rk3562_dclk_vop_set_clk(priv, clk->id, rate);
		break;
	case SCLK_GMAC0:
	case CLK_MAC0_2TOP:
	case CLK_MAC0_REFOUT:
		ret = rk3562_gmac_src_set_clk(priv, 0, rate);
		break;
	case CLK_MAC0_OUT:
		ret = rk3562_gmac_out_set_clk(priv, 0, rate);
		break;
	case SCLK_GMAC0_RX_TX:
		ret = rk3562_gmac_tx_rx_set_clk(priv, 0, rate);
		break;
	case CLK_GMAC0_PTP_REF:
		ret = rk3562_gmac_ptp_ref_set_clk(priv, 0, rate);
		break;
	case SCLK_GMAC1:
	case CLK_MAC1_2TOP:
	case CLK_MAC1_REFOUT:
		ret = rk3562_gmac_src_set_clk(priv, 1, rate);
		break;
	case CLK_MAC1_OUT:
		ret = rk3562_gmac_out_set_clk(priv, 1, rate);
		break;
	case SCLK_GMAC1_RX_TX:
		ret = rk3562_gmac_tx_rx_set_clk(priv, 1, rate);
		break;
	case CLK_GMAC1_PTP_REF:
		ret = rk3562_gmac_ptp_ref_set_clk(priv, 1, rate);
		break;
	case DCLK_EBC:
		ret = rk3562_ebc_set_clk(priv, rate);
		break;
	case ACLK_RKVDEC_PRE:
	case ACLK_RKVDEC:
	case CLK_RKVDEC_CORE:
		ret = rk3562_rkvdec_set_clk(priv, clk->id, rate);
		break;
	case TCLK_WDT_NS:
		ret = OSC_HZ;
		break;
...
static int rk3562_clk_set_parent(struct clk *clk, struct clk *parent)
{
	switch (clk->id) {
	case SCLK_GMAC0:
		return rk3562_gmac0_src_set_parent(clk, parent);
	case SCLK_GMAC1:
		return rk3562_gmac1_src_set_parent(clk, parent);
	case SCLK_GMAC0_RX_TX:
		return rk3562_gmac0_tx_rx_set_parent(clk, parent);
	case SCLK_GMAC1_RX_TX:
		return rk3562_gmac1_tx_rx_set_parent(clk, parent);
	case DCLK_VOP0:
	case DCLK_VOP1:
	case DCLK_VOP2:
		return rk3562_dclk_vop_set_parent(clk, parent);
	case ACLK_RKVDEC_PRE:
	case CLK_RKVDEC_CORE:
		return rk3562_rkvdec_set_parent(clk, parent);
...
#if (IS_ENABLED(OF_CONTROL)) || (!IS_ENABLED(OF_PLATDATA))
	.set_parent = rk3562_clk_set_parent,
#endif
};
...
static void rk3562_clk_init(struct rk3562_clk_priv *priv)
{
	...
	if (priv->armclk_init_hz != APLL_HZ) {
		ret = rk3562_armclk_set_clk(priv, APLL_HZ);
		if (!ret)
	...
	ret = rk3562_bus_set_clk(priv, ACLK_BUS, 150000000);
	if (ret < 0)
...
#if CONFIG_IS_ENABLED(OF_PLATDATA)
	.plat_auto	= sizeof(struct rk3562_clk_plat),
#endif
};
